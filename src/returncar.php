...
<head>
 <!-- Date 4/17/2020
  ...
  <!-- Date 5/4/2020
        ver 1.3 Brian Cabau: changed receipt.php header on line 218 to pay.php (receipt.php included in pay to calculate return or void return)
           -->
...
function dateDiff($start, $end) {
 ...
 WHERE c.car_ID=r.res_car_ID AND r.res_ID = '$id'";
 $result1 = $conn->query($sql1);
 if (mysqli_num_rows($result1) > 0) {
    while($row = mysqli_fetch_assoc($result1)) {
        $car_make = $row["car_make"];
        $car_model = $row["car_model"];
        $car_tagplate = $row["car_tagplate"];
        $car_price = $row["price"];
        $rent_start_date = $row["res_rentstart_date"];
        $rent_end_date = $row["res_rentend_date"];
        if ($today >= $rent_start_date && $today <= $rent_end_date){
          $no_of_days = dateDiff("$rent_start_date", "$today");
        } else if ($today > $rent_end_date){
...
if ($today > $rent_end_date){
  $extra_days = dateDiff("$rent_end_date", "$today");
} else {
  $extra_days = 0;
}


$duration = $no_of_days + $extra_days;
$total_fine = $duration*$car_price;
?>
    <div class="container" style="margin-top: 65px;" >
    <div class="col-md-7" style="float: none; margin: 0 auto;">
      <div class="form-area">
        <form role="form" action="pay.php?id=<?php echo $id ?>" method="POST">
        <br style="clear: both">
          <h3 style="margin-bottom: 5px; text-align: center; font-size: 30px;"> Booking Details </h3>
          <h6 style="margin-bottom: 25px; text-align: center; font-size: 12px;"> Please save for future references </h6>

           <h5> Car Make/Model:&nbsp;  <?php echo $car_make." ".$car_model;?></h5>

           <h5> Vehicle Tag:&nbsp;  <?php echo$car_tagplate;?></h5>

           <h5> Rent start date:&nbsp;  <?php echo$rent_start_date;?></h5>

           <h5> Return Date:&nbsp;  <?php echo $today;?></h5>

           <h5>Price: <?php echo("$" . $car_price . "/day");?><h5> 

           <h5> Number of Day(s):&nbsp;  <?php echo $duration;?></h5>

           <h5> Total amount:&nbsp;  <?php echo $total_fine;?></h5>

            <input type="hidden" name="durations" value="<?php echo $no_of_days; ?>">
        
          <input type="hidden" name="price" value="<?php echo $car_price; ?>">
          

           <input type="submit" name="submit" <?php if ($today > $rent_start_date){?> value="Comfirm Return" <?php } else { ?> value="Comfirm Cancel" <?php }?> class="btn btn-success pull-right">  
